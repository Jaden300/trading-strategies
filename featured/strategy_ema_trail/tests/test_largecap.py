"""
test_largecap.py — EMA Trail optimizer, cross-asset large-cap tech.

Usage:
    python strategy_ema_trail/tests/test_largecap.py
    python strategy_ema_trail/tests/test_largecap.py --jobs 4
"""
...
from strategy_ema_trail import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_ema_trail_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    "fast_len":          [15, 20, 25, 30],
    "slow_len":          [75, 100, 150, 200],
    "tp_pct":            [0.25, 0.30, 0.40],
    "sl_pct":            [0.08, 0.10, 0.15],
    "trail_trigger_pct": [0.06, 0.08, 0.10],
    "trail_step_pct":    [0.04, 0.05, 0.07],
}
...
def download_all(tickers: list[str], warmup_start: str, trade_start: str, end: str) -> dict:
...
def grid_search(all_data: dict, n_jobs: int = -1) -> pd.DataFrame:
    ...
    if n_jobs == 1:
        ...
        with mp.Pool(workers) as pool:
            results = list(tqdm(pool.imap(_worker, arg_list, chunksize=16),
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 140}")
    print(f"  TOP {n} EMA TRAIL COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe + 4×win_rate)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "fast_len", "slow_len", "tp_pct", "sl_pct",
        "trail_trigger_pct", "trail_step_pct",
        "score", "mean_sharpe", "min_sharpe", "mean_return",
        "mean_drawdown", "mean_win_rate", "total_trades", "mean_hold_bars",
    ] if c in df.columns]
    ...
    best = df.iloc[0]
    best_params = {k: (int(best[k]) if k in ("fast_len", "slow_len") else float(best[k]))
                   for k in PARAM_GRID}
    ...
    for ticker, entry in all_data.items():
        m = run_backtest(entry["df"], best_params, INITIAL_CAPITAL, COMMISSION, RISK_PCT,
                         trade_start_idx=entry["trade_start_idx"])
...
def main() -> None:
    p = argparse.ArgumentParser(description="EMA Trail — large-cap optimizer")
    ...
    print(f"Capital: ${INITIAL_CAPITAL:,.0f}  |  Risk: {RISK_PCT*100:.0f}% per trade  |  "
          f"Commission: {COMMISSION*100:.1f}% per leg")
