"""
test_largecap.py — Tetrad Elektro optimizer, cross-asset across large-cap tech stocks.

Bollinger Band mean-reversion: buy extreme oversold dips, sell upper-band recovery.

Usage:
    python strategy_elektro/tests/test_largecap.py
    python strategy_elektro/tests/test_largecap.py --jobs 4
    python strategy_elektro/tests/test_largecap.py --tickers TSLA NVDA AAPL
"""
...
from strategy_elektro import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_elektro_largecap.csv"
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 140}")
    print(f"  TOP {n} ELEKTRO COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "bb_length", "bb_mult", "rsi_length", "rsi_oversold", "rsi_exit",
    ...
    print(top.to_string(float_format=lambda x: f"{x:7.3f}", max_colwidth=10))
    print(f"{'─' * 140}\n")
...
def main() -> None:
    p = argparse.ArgumentParser(description="Tetrad Elektro — large-cap BB mean-reversion optimizer")
