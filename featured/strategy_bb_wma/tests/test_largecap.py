"""
test_largecap.py — Bollinger WMA optimizer, cross-asset large-cap tech.

Usage:
    python strategy_bb_wma/tests/test_largecap.py
    python strategy_bb_wma/tests/test_largecap.py --jobs 4
"""
...
from strategy_bb_wma import run_backtest

RESULTS_FILE = Path(__file__).parent.parent / "result_bb_wma_largecap.csv"
...
PARAM_GRID: dict[str, list] = {
    "bb_length": [50, 75, 100, 150, 200],
    "std_buy":   [0.3, 0.5, 0.8, 1.0, 1.5],
    "std_sell":  [1.5, 2.0, 2.5, 3.0],
}
...
def grid_search(all_data: dict, n_jobs: int = -1) -> pd.DataFrame:
    ...
    if n_jobs == 1:
        ...
        with mp.Pool(workers) as pool:
            results = list(
                tqdm(pool.imap(_worker, arg_list, chunksize=4),
...
def print_top(df: pd.DataFrame, all_data: dict, n: int = 10) -> None:
    print(f"\n{'─' * 140}")
    print(f"  TOP {n} BOLLINGER WMA COMBINATIONS  (score = mean_sharpe − 0.5×std_sharpe + 4×win_rate)")
    print(f"{'─' * 140}")
    cols = [c for c in [
        "bb_length", "std_buy", "std_sell",
    ...
    best_params = {
        ...
        "std_buy":   float(best["std_buy"]),
        "std_sell":  float(best["std_sell"]),
    }
...
def main() -> None:
    p = argparse.ArgumentParser(description="Bollinger WMA — large-cap optimizer")
